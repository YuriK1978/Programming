...
!!! генэксп - одноразовый, израсходовался (бросает ошибку StopIteration) и надо писать новый, в отличие от компсов.
...
9) использовавть компс, кодга нужна длина len или индексы.
...
def some_source():
    print("!!!")
    sleep(3)
    return[1, 2, 3] # если поставить return [1], то генератор упадет с ошибкой: int object is not iterable.
    
# Хотя генератор ленивый, источник он проверит при создании.
# Ему важно, что источник можно перебирать

if __name__ == '__main__':
    gen = (e for e in some_source()) 
# Здесь на выходе: !!!, а затем задержка 3 сек, затем окончание.
